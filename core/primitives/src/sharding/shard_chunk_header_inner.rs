...
use near_primitives_core::types::{Balance, BlockHeight, Gas, MerkleHash, ShardId};
...
impl ShardChunkHeaderInner {
    ...
    pub fn congestion_info(&self) -> Option<CongestionInfo> {
        match self {
            Self::V1(_) => None,
            Self::V2(_) => None,
            Self::V3(inner) => Some(inner.congestion_info),
            Self::V4(inner) => Some(inner.congestion_info),
        }
    }
...
// V3 -> V4: Add contract changes root.
#[derive(BorshSerialize, BorshDeserialize, Clone, PartialEq, Eq, Debug, ProtocolSchema)]
