...
impl Trie {
    ...
    pub fn validate_state_part(
        ...
        let storage = trie.trie_storage.as_partial_storage().unwrap();
...
mod tests {
    ...
    fn boundary_is_state_key() {
        ...
        for part_id in 1..num_parts {
            ...
            assert_matches!(trie.get_impl(&key_boundary), Ok(Some(_)));
        }
    ...
    fn test_combine_empty_trie_parts() {
        ...
        let contract_storage = Arc::new(ContractStorage::new(store.contract_store()));
        let _ =
        ...
        F: FnOnce(&mut ThreadRng, u64, u64) -> Vec<(Vec<u8>, Option<Vec<u8>>)>,
    {
        ...
        for num_parts in [2, 3, 5, 10, 50].iter().cloned() {
            ...
            for part_id in 0..num_parts {
                ...
                if part_id != 0 {
                    assert_matches!(trie.get_impl(&left_key_boundary), Ok(Some(_)));
                }
