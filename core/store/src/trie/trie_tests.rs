    ...
    Out: PartialEq + Debug,
{
    ...
    for i in 0..(size + 1) {
        let trie_storage = IncompletePartialStorage::new(storage.clone(), i);
        let new_trie = Trie::new(Arc::new(trie_storage), *trie.get_root(), None);
    ...
    for _ in 0..50 {
...

        {
            ...
            let lookup_test = |trie: Trie| -> Result<_, StorageError> {
                trie.get_impl(key).map(move |v| (trie, v))
            };
    ...
    fn get_touched_nodes_numbers(trie: &Trie, items: &[(Vec<u8>, Option<Vec<u8>>)]) -> Vec<u64> {
        items
            ...
            .map(|(key, value)| {
                ...
                let got_value = trie.get_impl(key).unwrap();
